...
CREATE PROC USP_Exportar_Percepcion_PDT_621
@Emp_cCodigo CHAR(3),
@Pan_cAnio CHAR(4),
@Per_cPeriodo CHAR(2),
@Moneda CHAR(3)
...
BEGIN
	...
	SELECT (ce.Ent_nRuc + @Separador + asd_cSerieDoc + @Separador + Asd_cNumDoc + @Separador + CONVERT(NCHAR(10), Asd_dFecDoc, 103) + @Separador + CAST(CAST(Asd_nHaber AS NUMERIC(12, 2)) AS VARCHAR(50)) + @Separador + RTRIM(LTRIM(Asd_cTipoDocRef)) + @Separador + RTRIM(LTRIM(Asd_cSerieDocRef)) + @Separador + RTRIM(LTRIM(Asd_cNumDocRef)) + @Separador +
	       CONVERT(NCHAR(10), Asd_dFecDocRef, 103) + @Separador + CAST(CAST(Asd_nDebe AS NUMERIC(12, 2)) AS VARCHAR(50)) + @Separador) AS Registro FROM #TMPPERCEPCION T
	LEFT JOIN dbo.CNM_ENTIDAD CE ON T.Emp_cCodigo = CE.Emp_cCodigo AND T.Ent_cCodEntidad = CE.Ent_cCodEntidad AND T.Ten_cTipoEntidad = CE.Ten_cTipoEntidad
	LEFT JOIN dbo.TABLA T2 ON CE.Emp_cCodigo = T2.Emp_cCodigo AND CE.Ent_cTipoDoc = T2.Tab_cCodigo AND T2.Tab_cTabla = '003'
	WHERE t.Asd_nHaber > 0 AND t.Per_cPeriodo = @Per_cPeriodo --AND (CreditoFiscal <> '0' OR MaterialConstruccion <> '0' OR PorcentajeSunat <> '0')
