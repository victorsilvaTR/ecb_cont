...
lArrMnt(2) = gsEmpresa
lArrMnt(3) = gsAnio
lArrMnt(4) = IIf(Me.tdbcLibro.BoundText = lsLibroTransCancAutomatico, "12", "13")
...
Private Sub cmdGenerarCierre_Click()
    ...
    If optAsiento(0).Value = True Then
        If Me.tdbcLibro.BoundText = lsLibroTransCancAutomatico Then
            respuesta = MsgBox(" " & Salto(2) & "Desea Generar el asiento de Transferencia y Cancelacion del Año Actual" & Salto(1) & "En el periodo de DICIEMBRE", vbYesNo + vbQuestion, "Confirmar Generar Asiento de Cierre")
        Else
...
'        lArrMnt(5) = "60"
        ...
        If Me.tdbcLibro.BoundText = lsLibroTransCancAutomatico Then
            ...
            If gstrCuentaCostoVenta = "69" Then
                sql1 = "USP_SPCN_CierreCuenta69 '12', '" & gsAnio & "', '" & gsEmpresa & "'"
                ObjFuncion.pEjecutaSQL (sql1)
                'Cuenta 79
                sql1 = "SPCN_CierreCuenta79 '" & gsEmpresa & "', '" & gsAnio & "', '12'"
                ObjFuncion.pEjecutaSQL (sql1)
            End If
            ...
            DoEvents
            Screen.MousePointer = vbDefault
            Mensajes "Los asientos de Transferencia y Cancelacion del ejercicio se crearon correctamente en el periodo de Diciembre", vbInformation
            cmdGenerarCierre.Enabled = True
        ...
        'Si la Cuenta Costo Venta es diferente de 69, el Cierre de estas cuentas se almacenaran en el Libro Cierre
        If gstrCuentaCostoVenta <> "69" Then
...
Private Sub Form_Load()
    ...
    sqlcombos = "SELECT LIB_CTIPOLIBRO, LIB_CDESCRIPCION FROM CNT_LIBRO_OPERA " & _
                "WHERE EMP_CCODIGO =  '" & gsEmpresa & "' AND (LIB_CTIPOLIBRO ='" & lsLibroCierre & "' OR LIB_CTIPOLIBRO = '" & lsLibroTransCancAutomatico & "') AND Pan_cAnio = '" & gsAnio & "' ORDER BY LIB_CDESCRIPCION "
...
Private Sub tdbcLibro_SelChange(Cancel As Integer)
    If Me.tdbcLibro.BoundText <> lsLibroTransCancAutomatico Then
        Me.Label1(1).Caption = "Seleccione el Libro donde se generarán los asientos automáticos, los asientos se crearan en el mes de AJUSTE"
    Else
        Me.Label1(1).Caption = "Seleccione el Libro donde se generarán los asientos automáticos, los asientos se crearan en el mes de DICIEMBRE"
    End If
End Sub
