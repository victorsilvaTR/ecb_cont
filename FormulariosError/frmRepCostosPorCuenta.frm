...
Begin VB.Form frmRepCostosPorCuenta 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Costos por Cuenta"
   ClientHeight    =   2355
   ClientLeft      =   7230
   ClientTop       =   4935
   ClientWidth     =   5190
   Icon            =   "frmRepCostosPorCuenta.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ScaleHeight     =   2355
   ...
   Begin VB.Frame fraTodo 
      ...
      Top             =   15
      Width           =   4980
      Begin VB.Frame FraTIPODE 
         Caption         =   "Columnas en el Reporte"
         Height          =   720
         Left            =   330
         TabIndex        =   1
         Top             =   210
         Width           =   3375
         Begin MSForms.CheckBox ChkSalAcum 
            Height          =   255
            Left            =   2100
            TabIndex        =   10
            Top             =   300
         ...
         End
         Begin MSForms.CheckBox ChkSalMes 
            Height          =   255
            Left            =   345
            TabIndex        =   9
            Top             =   300
      ...
      Begin TrueOleDBList70.TDBCombo tdbcMes 
         ...
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRepCostosPorCuenta.frx":0ECA
      ...
      Begin TrueOleDBList70.TDBCombo tdbcMoneda 
         ...
         RowSubDividerColor=   12632256
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRepCostosPorCuenta.frx":0F51
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
      ...
      Begin MSForms.CommandButton cmdImprimir 
         ...
         PicturePosition =   327683
         Size            =   "2355;767"
         Picture         =   "frmRepCostosPorCuenta.frx":0FD8
         FontHeight      =   165
         FontCharSet     =   0
         FontPitchAndFamily=   2
         ParagraphAlign  =   3
   ...
   End
End
Attribute VB_Name = "frmRepCostosPorCuenta"
...
Private Sub cmdImprimir_Click()
    If CE(tdbcMes.Text) = "" Then
        Mensajes "Seleccione un periodo"
        Exit Sub
    ...
    'matriz_fecha(0) = "@Tipo;RESUMEN_AL;True"
    ...
    matriz_fecha(3) = "@Mon_cMNac;" & IIf(tdbcMoneda.BoundText = "038", "1", "2") & ";True"
    matriz_fecha(4) = "@Moneda;" & tdbcMoneda.BoundText & ";True"
    matriz_fecha(5) = "@EMPRESA;" & gsEmpresaNom & ";True"
    matriz_fecha(6) = "@RUC;" & "RUC : " & gsRUC & ";True"
    matriz_fecha(7) = "@VerMes;" & IIf(ChkSalMes.Value = True, "1", "0") & ";True"
    matriz_fecha(8) = "@VerAcum;" & IIf(ChkSalAcum.Value = True, "1", "0") & ";True"
    matriz_fecha(9) = "@MostrarMes;" & IIf(ChkSalMes.Value = True, "1", "0") & ";True"
    matriz_fecha(10) = "@MostrarAcum;" & IIf(ChkSalAcum.Value = True, "1", "0") & ";True"
    
    ' *** Verificando Grupo
    Dim formulas(0) As Variant
    
    formulas(0) = "grupo = {spCn_RptCentroCostosResumen;1.Cos_cCodigo}"



    AbreReporteParam gsDSN, Me, rutaReportes & "RptCentroCosto_x_Cuenta.rpt", crptToWindow, "Costos por Cuenta", "", matriz_fecha(), formulas()
...
Private Sub Form_Load()
    ...
    ElseIf gsMes = "13" Or gsMes = "14" Then
        tdbcMes.BoundText = "12"
    Else
        tdbcMes.BoundText = gsMes
    End If
...
Private Sub Form_Unload(Cancel As Integer)
    Set frmRepResumenCentrocosto = Nothing
...
Private Sub tdbcMoneda_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = 13 Then pSetFocus cmdImprimir
End Sub
