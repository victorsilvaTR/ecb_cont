...
Else
    ...
    Exit Sub
End If
'If Month(fechahoy) = 1 Then mes_tmp = "12"
'mes_fichero = Left(Right(LstLE.Text, 20), 2)
'anio_fichero = Left(Right(LstLE.Text, 24), 4)
    'If (Year(fechahoy) = Val(anio_fichero) And mes_fichero <= Month(fechahoy)) Or (Val(mes_fichero) = Val(mes_tmp) And Val(anio_fichero) >= Year(fechahoy) - 1) Then
If (Year(fecha) = Year(fechahoy) And Month(fecha) = Month(fechahoy)) Then
    ...
    End If
Else
    Mensajes "Solo puede eliminar Libros Electrónicos que se hayan generado hasta con un mes anterioridad.", vbExclamation
End If
Set clDatos = Nothing
Set rsAddItem = Nothing
Exit Sub
...
mes_fichero = Left(Right(sArch, 20), 2)


Do While sArch <> ""
'    nombre_fichero = Path.GetFileName(sArch)
    mes_fichero = Left(Right(sArch, 20), 2)
    anio_fichero = Left(Right(sArch, 24), 4)
