...
Begin VB.Form frmPrcImportarDatosXLS 
   ...
   Begin VB.Frame fraTodo 
      ...
      Begin MSForms.CommandButton cmdRefresh 
         ...
         Size            =   "794;688"
         Picture         =   "frmPrcImportarDatosDbf.frx":1066
         FontHeight      =   165
         FontCharSet     =   0
         FontPitchAndFamily=   2
         ParagraphAlign  =   3
...
On Error GoTo Control
    ...
    cParametros = CE(chkOpcion(0).Value) & _
                  CE(chkOpcion(1).Value) & _
                  CE(chkOpcion(2).Value) & _
                  CE(chkOpcion(3).Value) & _
                  CE(chkOpcion(4).Value) & _
                  CE(chkOpcion(5).Value) & _
                  CE(chkOpcion(6).Value) & _
                  CE(chkOpcion(7).Value)
...
'    Set Wb = Ex.Workbooks.Open(Me.dlgAbrirArchivo.FileName)
    ...
    'If CrearTablas(Me.dlgAbrirArchivo.FileName, pbAvance, lblAvance, cParametros) Then
        ...
        If chkOpcion(2).Value = 1 Then
            ...
            "[Asd_nDebeSoles] NUMERIC (14,2) NULL , " & vbCrLf & "[Asd_nHaberSoles] NUMERIC (14,2) NULL , " & vbCrLf & _
            "[Asd_nTipoCambio] NUMERIC (14,2) NULL , " & vbCrLf & "[Asd_nDebeMonExt] NUMERIC (14,2) NULL , " & vbCrLf & _
            "[Asd_nHaberMonExt] NUMERIC (14,2) NULL , " & vbCrLf & "[Cos_cCodigo] varchar (12) NULL, " & vbCrLf & _
            ...
            For VarContFila = 2 To 70000
                ...
                sSql = "Insert Into ZIMP_APE_DET(Ase_cNummov, Pan_cAnio, Per_cPeriodo, " & _
                    ...
                    "', '" & IIf(IsNumeric(Sht.Range("S" & VarContFila).Value), Format(Val(Sht.Range("S" & VarContFila).Value), "0000"), Sht.Range("S" & VarContFila).Value) & "', '" & Sht.Range("T" & VarContFila).Value & "', '" & IIf(Sht.Range("U" & VarContFila).Value = "", "", Format(Sht.Range("U" & VarContFila).Value, "dd/mm/yyyy")) & _
        ...
        If chkOpcion(3).Value = 1 Then ' Importar Compras
            ...
            "[Ase_cEstadoO] char (1) NULL , " & vbCrLf & " [Ase_cEstadoD] char (1) NULL" & vbCrLf & ")"
            Call cn.Execute(VarSql)
...
'            cn.Execute ("set dateformat dmy")
            For VarContFila = 2 To 70000
                If Sht.Range("A" & VarContFila).Value = "" Then Exit For
                pbAvance.Value = pbAvance.Value + 1
                PeriodoXls = Format(Val(Sht.Range("C" & VarContFila).Value), "00")
                TipoLibroXls = Format(Val(Sht.Range("D" & VarContFila).Value), "00")
                pbAvance.Refresh
                DoEvents
                cn.BeginTrans
                    '*****SI ENCUENTRA UNA SOLA COMILLA EN LA GLOSA*****
...
'                End If
                ...
                sSql = sSql & "', '" & IIf(IsNumeric(Sht.Range("S" & VarContFila).Value), Format(Val(Sht.Range("S" & VarContFila).Value), "0000"), Sht.Range("S" & VarContFila).Value) & "', '" & IIf(IsNumeric(Sht.Range("T" & VarContFila).Value), Format(Val(Sht.Range("T" & VarContFila).Value), "00000000"), vbNullString) & "', '" & IIf(Sht.Range("U" & VarContFila).Value = "", "", Format(Sht.Range("U" & VarContFila).Value, "dd/MM/yyyy"))
                
                sSql = sSql & "', '" & Sht.Range("V" & VarContFila).Value & "', '" & IIf(Sht.Range("W" & VarContFila).Value = "", "", Format(Sht.Range("W" & VarContFila).Value, "dd/MM/yyyy")) & "', '" & Format(Val(Sht.Range("X" & VarContFila).Value), "0000")
        ...
        If chkOpcion(4).Value = 1 Then 'Importar Ventas
            ...
            "[Per_cPeriodo] char (2) NULL, " & vbCrLf & "[Lib_cTipoLibro] char (2) NULL, " & vbCrLf & _
            "[Ase_nVoucher] char (10) NULL, " & vbCrLf & "[Ase_dFecha] nvarchar (255) NULL, " & vbCrLf & _
            "[Ase_cGlosa] nvarchar (255) NULL, " & vbCrLf & "[Ase_cTipoMoneda] nvarchar (255) NULL," & vbCrLf & _
            "[Ase_cEstadoO] char (1) NULL , " & vbCrLf & " [Ase_cEstadoD] char (1) NULL" & vbCrLf & ")"
            Call cn.Execute(VarSql)
            ...
            VarSql = "CREATE TABLE [" & sBaseDatos & "].[dbo].[ZIMP_VENTAS_DET] (" & vbCrLf & _
                        ...
                        "[Asd_cOperaTC] char (3) NULL, " & vbCrLf & "[Asd_cTipoMoneda] char (3) NULL, " & vbCrLf & _
                        "[Asd_cTipoDocRef] nvarchar (255) NULL, " & vbCrLf & _
                        "[Asd_dFecDocRef] datetime NULL, " & vbCrLf & "[Asd_cSerieDocRef] nvarchar (255) NULL, " & vbCrLf & _
                        "[Asd_cNumDocRef] nvarchar (255) NULL )"
            ...
            For VarContFila = 2 To 70000
                ...
                DoEvents
                    ...
                    sSql = sSql & "Lib_cTipoLibro, Ase_nVoucher, Pla_cCuentaContable, Asd_nItem, Asd_cGlosa, Asd_nDebeSoles, Asd_nHaberSoles, Asd_nTipoCambio, "
                    sSql = sSql & "Asd_nDebeMonExt, Asd_nHaberMonExt, Cos_cCodigo, Ten_cTipoEntidad, Ent_cCodEntidad, Asd_cTipoDoc, Asd_dFecDoc, Asd_cSerieDoc, Asd_cNumDoc, "
                    sSql = sSql & "Asd_dFecVen, Asd_nMontoInafecto, Asd_cBaseImp, Asd_cProvCanc, Asd_cOperaTC, Asd_cTipoMoneda, Asd_cTipoDocRef, Asd_dFecDocRef, Asd_cSerieDocRef, Asd_cNumDocRef) Values "
                    ...
                    sSql = sSql & "', '" & IIf(IsNumeric(Sht.Range("S" & VarContFila).Value), Format(Val(Sht.Range("S" & VarContFila).Value), "0000"), Sht.Range("S" & VarContFila).Value) & "',"
                    ...
                    sSql = sSql & "', '" & Sht.Range("Y" & VarContFila).Value & "', '" & Format(Val(Sht.Range("Z" & VarContFila).Value), "000") & "', " & _
                           "'" & Sht.Range("AA" & VarContFila).Value & "', '" & Sht.Range("AB" & VarContFila).Value & "', '" & Sht.Range("AC" & VarContFila).Value & "', '" & Sht.Range("AD" & VarContFila).Value & "')"
                    Call cn.Execute(sSql)
                cn.CommitTrans

                cn.BeginTrans
                
                sSql = "update ZIMP_VENTAS_CAB set Ase_cEstadoO = " & IIf(Trim(Sht.Range("H" & VarContFila).Value) = "ANULADO", "2", "1")
                sSql = sSql & " ,Ase_cEstadoD = ''"
                sSql = sSql & " Where Pan_cAnio = '" & gsAnio & "' and Per_cPeriodo = '" & Format(Val(Sht.Range("C" & VarContFila).Value), "00") & "'"
                sSql = sSql & " and Per_cPeriodo = '" & Format(Val(Sht.Range("C" & VarContFila).Value), "00") & "' and ase_nvoucher = '" & Format(Val(Sht.Range("E" & VarContFila).Value), "0000000000") & "'"
                cn.Execute (sSql)
                cn.CommitTrans
        ...
        If chkOpcion(5).Value = 1 Then
            ...
            For VarContFila = 2 To 70000
                ...
                cn.BeginTrans
                    ...
                    sSql = sSql & "', '" & IIf(LTrim(RTrim(Sht.Range("S" & VarContFila).Value)) = "", "", Format(Val(Sht.Range("S" & VarContFila).Value), "0000")) & "', '" & IIf(LTrim(RTrim(Sht.Range("T" & VarContFila).Value)) = "", "", Sht.Range("T" & VarContFila).Value) & "', '" & IIf(LTrim(RTrim(Sht.Range("U" & VarContFila).Value)) = "", "", Format(Sht.Range("U" & VarContFila).Value, "dd/mm/yyyy"))
        ...
        If chkOpcion(6).Value = 1 Then
            ...
            For VarContFila = 2 To 70000
                ...
                cn.BeginTrans
                    ...
                    "', '" & IIf(IsNumeric(Sht.Range("S" & VarContFila).Value), Format(Val(Sht.Range("S" & VarContFila).Value), "0000"), Sht.Range("S" & VarContFila).Value) & "', '" & IIf(Sht.Range("T" & VarContFila).Value = "", "", Sht.Range("T" & VarContFila).Value) & "', '" & Sht.Range("U" & VarContFila).Value & _
        ...
        If chkOpcion(7).Value = 1 Then
            ...
            For VarContFila = 2 To 70000
                ...
                "', '" & Format(Val(Sht.Range("S" & VarContFila).Value), "0000") & "', '" & Sht.Range("T" & VarContFila).Value & "', '" & Sht.Range("U" & VarContFila).Value & _
                "', '" & Sht.Range("V" & VarContFila).Value & "', '" & Sht.Range("W" & VarContFila).Value & "', '" & Sht.Range("X" & VarContFila).Value & "')")
                cn.CommitTrans
            Next VarContFila
    End If
    
'    If cn.State <> 0 Then cn.CommitTrans
