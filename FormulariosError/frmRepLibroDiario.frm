...
DCOROI:
    ...
    End If
    If ReporteSunat = "F0501" And OptImpresion(1).Value Then
        AbreReporteParam gsDSN, Me, rutaReportes & "RptFormato_0501.rpt", crptToWindow, "Libro Diario", "", matriz_fecha(), formulas()
...
If (tdbcMes.BoundText = "00" Or tdbcMes.BoundText = "13" Or tdbcMes.BoundText = "14") And _
    ...
    OptImpresion(2).Value = False
    OptImpresion(3).Value = False
Else
   ...
   If .RecordCount > 0 Then
    ...
    Do While Not .EOF
     If Trim(!Per_cPeriodo) <> Trim(pPer_cPeriodo) And Trim(pPer_cPeriodo) <> "" And giLineas = 11 Then
      ...
      printl (Space(3) & Space(58) & "VIENEN..." & Space(25) & pVienenDebe & Space(1) & pVienenHaber)
     End If
      ...
      Do While Trim(!Per_cPeriodo) = Trim(pPer_cPeriodo)
       ...
       Do While Trim(!Per_cPeriodo) = Trim(pPer_cPeriodo) And Trim(!Ase_nVoucher) = Trim(pAse_nVoucher)
        ...
        pTotalMesHaber = CDbl(pTotalMesHaber) + CDbl(pHaber): RSet pTotalMesHaber = Format$(pTotalMesHaber, "#,###,###,##0.00;(#,###,###,##0.00)")
         ...
         gsLinea = (Space(3) & pAse_nVoucher & Space(3) & pAse_dFecha & Space(3) & pAsd_cGlosa & Space(3) & pAsd_cTipoDoc & pAsd_cSerieDoc & pAsd_cNumDoc & Space(2) & pPla_cCuentaContable & Space(1) & pDebe & Space(2) & pHaber)
        ...
        If giLineas >= 73 Then '71
            ...
            printl (Space(3) & Space(57) & "VAN..." & Space(29) & pVanDebe & Space(1) & pVanHaber)
            If giLineas = 75 Then printl ""
            
            pVienenDebe = pVanDebe
            pVienenHaber = pVanHaber
            
            'pVanDebe = CDbl(pVanDebe) + CDbl(pSumDebe)
            'pVanHaber = CDbl(pVanHaber) + CDbl(pSumHaber)
        ...
        If Not .EOF Then
            If Trim(!Per_cPeriodo) = Trim(pPer_cPeriodo) And Trim(!Ase_nVoucher) <> Trim(pAse_nVoucher) Then
                printl (Space(3) & Space(94) & "------------ -------------")
                printl (Space(3) & Space(91) & pSumDebeXA & pSumHaberXA)
        ...
        Else
            printl (Space(3) & Space(94) & "------------ -------------")
            printl (Space(3) & Space(91) & pSumDebe & pSumHaber)
            pVanDebe = CDbl(pVanDebe) + CDbl(pSumDebe): RSet pVanDebe = Format$(pVanDebe, "#,###,###,##0.00;(#,###,###,##0.00)")
            pVanHaber = CDbl(pVanHaber) + CDbl(pSumHaber): RSet pVanHaber = Format$(pVanHaber, "#,###,###,##0.00;(#,###,###,##0.00)")
            Exit Do
        End If
       ...
       If .EOF Then
        ...
        If LTrim(RTrim(NombreMes(pPer_cPeriodo))) <> "" Then
            printl (Space(3) & Space(57) & "TOTAL  " & NombreMes(pPer_cPeriodo) & Space(SpaceMes) & pTotalMesDebe & pTotalMesHaber)
        Else
            printl (Space(3) & Space(57) & "TOTAL  " & NombreMes(pPer_cPeriodo) & Space(SpaceMes) & pTotalMesDebe & pTotalMesHaber)
        End If
       ...
       ElseIf !Per_cPeriodo <> Trim(pPer_cPeriodo) Then
        ...
        If LTrim(RTrim(NombreMes(pPer_cPeriodo))) = "" Then
            printl (Space(3) & Space(SpaceMes + 72) & pTotalMesDebe & pTotalMesHaber) ': printl ""
        Else
            printl (Space(3) & Space(57) & "TOTAL " & NombreMes(pPer_cPeriodo) & Space(SpaceMes + 1) & pTotalMesDebe & pTotalMesHaber)
            pTotalMesDebe = 0
            pTotalMesHaber = 0
        End If
        pSumDebeXA = 0
        ...
        For i = giLineas To giLineasIni + (NroSpacesMVan - 3)
          ...
          Else
           ...
           RSet pVanHaber = Format$(pVanHaber, "#,###,###,##0.00;(#,###,###,##0.00)")
            ...
            End If
'           RSet pVanDebe = Format(CDbl(pVanDebe) + SumPerAnt_DebeSoles, "#,###,###,##0.00;(#,###,###,##0.00)")
'           RSet pVanHaber = Format(CDbl(pVanHaber) + SumPerAnt_HaberSoles, "#,###,###,##0.00;(#,###,###,##0.00)")
'           printl (Space(58) & "VAN..." & Space(28) & pVanDebe & Space(1) & pVanHaber)
'           SumPerAnt_DebeSoles = 0: SumPerAnt_HaberSoles = 0
           Exit For
          End If
        Next i
        Debug.Print "5"
        'If Not giLineas >= 75 Then giLineas = 75
        'printl ""
       End If
      Loop
      ...
      If .EOF Then
        ...
        printl (Space(3) & Space(57) & "TOTALES " & Space(26) & pTotalGenDebe & pTotalGenHaber)
...
End Function
Public Sub CabeceraLibroDiario()
On Error GoTo Control
EntroCabecera = True
...
' If Not gsNomTipoImp Then
 ...
 giLineas = 0
    ...
    'Call AlinearDosTextos(nAncho - 11, Space(3) & "Formato 5.1: LIBRO DIARIO", "Fecha :  " & Format(FechaServidor, "dd/MM/yyyy"))
    printl (Space(3) & "Formato 5.1: LIBRO DIARIO                                                                         Fecha :  " & Format(FechaServidor, "dd/MM/yyyy"))
    ...
    If rsArreglo.EOF Then
        Call AlinearDosTextos(nAncho, Space(3) & "EJERCICIO/PERIODO    : " & NombreMes(pPer_cPeriodo) & " " & gsAnio, "")
    Else
        If tdbcMes.BoundText = "00" Then
            VarTitulo = "APERTURA"
    ...
    Call AlinearDosTextos(nAncho - 9, Space(3) & "EJERCICIO/PERIODO    : " & VarTitulo, "")
        
    End If
    Dim xgsPagina As String * 4
    RSet xgsPagina = Format(CStr(gsPagina), "####")
    'Call AlinearDosTextos(nAncho - 18, Space(3) & "RUC                  : " & gsRUC, "Pagina: " & xgsPagina)
    printl (Space(3) & "RUC                  : " & gsRUC & "                                                                Pagina: " & xgsPagina)
    
    'Call AlinearDosTextos(nAncho, "RAZON SOCIAL         : " & gsEmpresaNom, "Página:       " & Format$(gsPaginaPrincipal, "####") & " de " & Format$(gsPagina, "####"))

    Call AlinearDosTextos(nAncho - 18, Space(3) & "APELLIDOS Y NOMBRES,", "")
    Call AlinearDosTextos(nAncho - 18, Space(3) & "DENOMINACIÓN O", "")
    Call AlinearDosTextos(nAncho - 18, Space(3) & "RAZON SOCIAL         : " & gsEmpresaNom, "")
    ...
    printl ("")
    printl (Space(3) & "------------ ------------ ------------------------------- --------------------------------------------------------------")
    printl (Space(3) & "   NUM.         FECHA               GLOSA O                  DOCUMENTO         CTA.CONTABLE            MOVIMIENTO")
    'printl ("  CORREL          DE               DESCRIPCIÓN            T.D. SER.    NÚMERO   CÓDIGO            DENOMINACIÓN         DEBE         HABER ")
    printl (Space(3) & "  CORREL         DE               DESCRIPCION DE          T.D. SER. NUMERO       CODIGO            DEBE           HABER ")
    printl (Space(3) & " COD.OPER.    OPERACION           LA OPERACION")
    'printl ("------------ ------------ ------------------------------- --- --- ------------ ------------ ----------------------- ------------ -------------")
    printl (Space(3) & "------------ ------------ ------------------------------- --- --- ------------ ------------ ------------ ---------------")
