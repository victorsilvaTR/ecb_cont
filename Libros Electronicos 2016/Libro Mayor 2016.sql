...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spCn_RptMayorElectronico3]
 @Emp_cCodigo char(3)='',              
 @Pan_cAnio char(4)='',              
...
Pla_cCuentaContable varchar(30),CUO varchar(30),Correlativo varchar(30), D3 varchar(10), D2 varchar(10), D2_sumas varchar(10),                                                              
 ...
 Asd_cEstadoO char(1),Asd_cEstadoD char(1), Id_Aduana char(10), Ent_TipoDoc char(2), Ent_cNroDoc varchar(15))'                                                      
 exec(@Query)
...
Asd_dFecDoc datetime, Asd_cTipoDocRef char(3), Asd_cSerieDocRef char(5), Asd_cNumDocRef char(20),                                                   
 ...
 Pla_cNombreCuenta varchar(120), Pla_cNombreCuentaD3 varchar(120),Pla_cNombreCuentaD2 varchar(100),Asd_cEstadoO char(1),Asd_cEstadoD char(1), Id_Aduana char(10), Ent_TipoDoc char(2), Ent_cNroDoc varchar(15))'                                                      
 exec(@Query)                                                      
...
Asd_cEstadoO, Asd_cEstadoD, CND_ASIENTO_VOUCHER.Id_Aduana, CNM_ENTIDAD.Ent_cTipoDoc, CNM_ENTIDAD.Ent_nRuc                                    
...
CND_ASIENTO_VOUCHER.Emp_cCodigo,    CND_ASIENTO_VOUCHER.Pan_cAnio, '''' AS Per_cPeriodo,   ''XX'' AS Lib_cTipoLibro,                                            
    ...
    Asd_cEstadoO, Asd_cEstadoD, CND_ASIENTO_VOUCHER.Id_Aduana, CNM_ENTIDAD.Ent_cTipoDoc, CNM_ENTIDAD.Ent_nRuc   FROM CND_ASIENTO_VOUCHER LEFT JOIN CNM_ENTIDAD ON CND_ASIENTO_VOUCHER.Emp_cCodigo = CNM_ENTIDAD.Emp_cCodigo AND            
    CND_ASIENTO_VOUCHER.Ten_cTipoEntidad = CNM_ENTIDAD.Ten_cTipoEntidad AND CND_ASIENTO_VOUCHER.Ent_cCodEntidad = CNM_ENTIDAD.Ent_cCodEntidad                                                             
 ...
 set @sql = @sql + ' GROUP BY CND_ASIENTO_VOUCHER.Pla_cCuentaContable,              
    ...
    CND_ASIENTO_VOUCHER.Pan_cAnio,Asd_cEstadoO, Asd_cEstadoD, CND_ASIENTO_VOUCHER.Id_Aduana, CNM_ENTIDAD.Ent_cTipoDoc, CNM_ENTIDAD.Ent_nRuc , CND_ASIENTO_VOUCHER.Asd_nDebeSoles, CND_ASIENTO_VOUCHER.Asd_nHaberSoles, CND_ASIENTO_VOUCHER.Asd_nDebeMonExt, CND_ASIENTO_VOUCHER.Asd_nHaberMonExt '              
...
CTAD2.Pla_cNombreCuenta AS Pla_cNombreCuentaD2 ,TMP.Asd_cEstadoO, TMP.Asd_cEstadoD, TMP.Id_Aduana, TMP.Ent_TipoDoc , TMP.Ent_cNroDoc                                                     
FROM ' + @tabla + ' as TMP                                                      
...
Asd_cSerieDoc, Asd_cNumDoc, '''' AS Ase_cGlosaRef, CASE WHEN Lib_cTipoLibro = ''05'' THEN ''140100&'' + convert(varchar(4),year(Asd_dFecDoc)) + Per_cPeriodo + ''&'' + LTRIM(RTRIM(LEFT(Ase_nVoucher,40))) + ''&'' + LEFT(LTRIM(RTRIM(''M'' + left(Ase_nVoucher,4) + right(Ase_nVoucher,5) )),100) 
			   ...
			   LEFT(LTRIM(RTRIM(''M'' + left(Ase_nVoucher,4) + right(Ase_nVoucher,5) )),100) ELSE '''' END AS Ase_cTipoLibro, Pan_cAnio, Per_cPeriodo AS Per_cPeriodoRef, Ase_nVoucher, Id_Aduana, Ent_TipoDoc, Ent_cNroDoc, Lib_cTipoLibro                                        
 from ' + @tabla2 + ''                                                     
exec(@sql) 
...
Asd_cTipoMoneda, Asd_cTipoDocEmisor, Asd_cNumEmisor, Asd_cTipoDoc, Asd_cSerieDoc, Asd_cNumDoc, Ase_cGlosaRef, Ase_cTipoLibro, Pan_cAnio, Per_cPeriodoRef, Ase_nVoucher, Id_Aduana, Asd_nDebeSoles, Asd_nHaberSoles, Ent_cTipoDoc, Ent_nRuc, TipoLibro                               
...
@Ent_cTipoDoc, @Ent_nRuc, @TipoLibro                                                
...
@Asd_cTipoMoneda, @Asd_cTipoDocEmisor, @Asd_cNumEmisor, @Asd_cTipoDoc, @Asd_cSerieDoc, @Asd_cNumDoc, @Ase_cGlosaRef, @Ase_cTipoLibro, @Pan_cAnio, @Per_cPeriodoRef, @Ase_nVoucher, @Id_Aduana, Ent_cTipoDoc, Ent_nruc, @TipoLibro  from TMPDiarioPLEDH         
...
--exec(@sql)                                                     
       ...
       UPDATE TMPDiarioPLEDH
		SET Ent_cTipoDoc = T.Tab_cCodSunat
       FROM dbo.TABLA T
       WHERE T.Emp_cCodigo = @Emp_cCodigo AND T.Tab_cTabla = '003' AND t.Tab_cCodigo = TMPDiarioPLEDH.Ent_cTipoDoc
       
       UPDATE TMPDiarioPLEDH
		SET Ent_cTipoDoc = ISNULL(Ent_cTipoDoc, ''), Ent_nRuc = ISNULL(Ent_nRuc, ''), Id_Aduana = ISNULL(Id_Aduana, '')
...
SELECT (LTRIM(RTRIM(Per_cPeriodo)) + @Sepa + LTRIM(RTRIM(LEFT(CUO,40))) + @Sepa + 
        ...
        + @Sepa + LEFT(Pla_cCuentaContable, 24) + @Sepa + '' + @Sepa + '' + @Sepa +  Asd_cTipoMoneda + @Sepa + 
        CASE WHEN TipoLibro IN ('05', '02') THEN Asd_cTipoDocEmisor
             WHEN TipoLibro IN ('06', '04') THEN RTRIM(LTRIM(Ent_cTipoDoc)) ELSE '' END + @Sepa + 
        CASE WHEN TipoLibro IN ('05', '02') THEN RTRIM(LTRIM(Asd_cNumEmisor))
             WHEN TipoLibro IN ('06', '04') THEN Ent_nRuc ELSE '' END + @Sepa + 
        RTRIM(LTRIM(Asd_cTipoDoc)) + @Sepa + 
