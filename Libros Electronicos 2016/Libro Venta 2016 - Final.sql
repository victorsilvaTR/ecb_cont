 ...
 @vAsd_cBaseImp, @vAsd_dFecVen, @Asd_cImpAdic                                              
  ...
  Begin                                                
   ...
   Begin                                                
    ...
    IF LEFT(@vPla_cCuentaContable, 2) = '12' OR LEFT(@vPla_cCuentaContable, 2) = '16'  
    BEGIN  
...
BEGIN  
       SELECT (CASE WHEN (REP.Asd_cEstadoO = '8' OR REP.Asd_cEstadod = '8') THEN CAST(YEAR(REP.Asd_dFecDoc) AS CHAR(4)) ELSE convert(varchar(4),year(REP.Ase_dFecha)) END + CASE WHEN (REP.Asd_cEstadoO = '8' OR REP.Asd_cEstadod = '8') THEN RIGHT(100 + DATEPART(MM,REP.Asd_dFecDoc), 2) ELSE REP.Per_cPeriodo END + '00' + @Separador +   
               LTRIM(RTRIM(LEFT(REP.Ase_nVoucher,40))) + @Separador +  
                ...
                + (SELECT t.Tab_cCodSunat FROM dbo.TABLA T WHERE T.Emp_cCodigo = REP.Emp_cCodigo AND T.Tab_cTabla = '003' AND T.Tab_cCodigo = CE.Ent_cTipoDoc) + @Separador + RTRIM(LTRIM(CE.Ent_nRuc)) + @Separador + RTRIM(LTRIM(CE.Ent_cPersona)) + @Separador +   
                CAST(CAST(REP.MontoExp AS NUMERIC(18, 2)) AS VARCHAR(50)) + @Separador +   
                  
                CASE WHEN LEFT(REP.Asd_cTipoDoc,2) IN ('07', '87') AND 
                              (MONTH(REP.Asd_dFecDocRef) = CAST(@Per_cPeriodo AS INT) AND YEAR(REP.Asd_dFecDocRef) = @Pan_cAnio) THEN CAST(CAST(REP.MontoBaseGra AS NUMERIC(18, 2)) AS VARCHAR(50))   
                     WHEN LEFT(REP.Asd_cTipoDoc,2) NOT  IN ('07', '87') THEN CAST(CAST(REP.MontoBaseGra AS NUMERIC(18, 2)) AS VARCHAR(50))   
                     ELSE '0.00' END + @Separador +   
                       
                CASE WHEN LEFT(REP.Asd_cTipoDoc, 2) IN ('07', '87') AND 
                              (MONTH(REP.Asd_dFecDocRef) <> CAST(@Per_cPeriodo AS INT) OR YEAR(REP.Asd_dFecDocRef) <> @Pan_cAnio) THEN CAST(CAST(REP.MontoBaseGra AS NUMERIC(18, 2)) AS VARCHAR(50))  ELSE '0.00' END
+ @Separador +  
                  
                CASE WHEN LEFT(REP.Asd_cTipoDoc,2) IN ('07', '87') AND (MONTH(REP.Asd_dFecDocRef) = CAST(@Per_cPeriodo AS INT) AND YEAR(REP.Asd_dFecDocRef) = @Pan_cAnio) THEN CAST(CAST(REP.MontoIGV AS NUMERIC(18, 2)) AS VARCHAR(50))
                     WHEN LEFT(REP.Asd_cTipoDoc,2) NOT IN ('07', '87') THEN CAST(CAST(REP.MontoIGV AS NUMERIC(18, 2)) AS VARCHAR(50))
                     ELSE '0.00' END + @Separador + 
  
                CASE WHEN LEFT(REP.Asd_cTipoDoc, 2) IN ('07', '87') AND (MONTH(REP.Asd_dFecDocRef) <> CAST(@Per_cPeriodo AS INT) OR YEAR(REP.Asd_dFecDocRef) <> @Pan_cAnio) 
                     THEN CAST(CAST(REP.MontoIGV AS NUMERIC(18, 2)) AS VARCHAR(50)) ELSE '0.00' END + @Separador +  
                ...
                ctm.Mon_cCodSunat + @Separador + CASE WHEN ctm.Mon_cCodSunat = 'PEN' THEN '1.000' ELSE CAST(CAST(REP.Asd_nTipoCambio AS NUMERIC(4, 3)) AS VARCHAR(50)) END + @Separador +   
                CASE  WHEN LEFT(REP.Asd_cTipoDoc,2) IN ('07', '08', '87', '88') THEN convert(varchar(10),REP.Asd_dFecDocRef,103) ELSE '' END + @Separador +  
...
BEGIN  
       ...
       CAST(CAST(REP.MontoBaseGra AS NUMERIC(18, 2)) AS VARCHAR(50)) + @Separador +   
       CAST(CAST(REP.MontoIGV AS NUMERIC(18, 2)) AS VARCHAR(50)) + @Separador +   
         
       CAST(CAST(REP.MontoOtros AS NUMERIC(18, 2)) AS VARCHAR(50)) + @Separador + CAST(CAST(REP.MontoTotal AS NUMERIC(18, 2)) AS VARCHAR(50)) + @Separador + ctm.Mon_cCodSunat + @Separador +  
       CASE WHEN ctm.Mon_cCodSunat = 'PEN' THEN '1.000' ELSE CAST(CAST(REP.Asd_nTipoCambio AS NUMERIC(4, 3)) AS VARCHAR(50)) END + @Separador +  
       CASE WHEN LEFT(REP.Asd_cTipoDoc,2) IN ('07', '08') THEN convert(varchar(10),REP.Asd_dFecDocRef,103) ELSE '' END + @Separador +   
       CASE WHEN LEFT(REP.Asd_cTipoDoc,2) IN ('07', '08') THEN LEFT(REP.Asd_cTipoDocRef,2) ELSE '' END + @Separador +  
       CASE WHEN LEFT(REP.Asd_cTipoDoc,2) IN ('07', '08') THEN RTRIM(REP.Asd_cSerieDocRef) ELSE '' END + @Separador + CASE WHEN LEFT(REP.Asd_cTipoDoc,2) IN ('07', '08') THEN RIGHT(RTRIM(REP.Asd_cNumDocRef), 7) ELSE '' END + @Separador + '' + @Separador + 
'' + @Separador + LTRIM(RTRIM(LEFT(case when REP.Asd_cEstadoD = '' then REP.Asd_cEstadoO else  REP.Asd_cEstadoD end,1))) + @Separador ) AS Registro FROM #TMPREGISTROLEVENTAS REP LEFT JOIN dbo.CNM_ENTIDAD CE  
       ON REP.Emp_cCodigo = CE.Emp_cCodigo AND CE.Ten_cTipoEntidad = REP.Ten_cTipoEntidad AND CE.Ent_cCodEntidad = REP.Ent_cCodEntidad  
       LEFT JOIN dbo.CNC_ASIENTO_VOUCHER CAV ON REP.Emp_cCodigo = CAV.Emp_cCodigo AND CAV.Ase_nVoucher = REP.Ase_nVoucher AND cav.Ase_cNummov = REP.Ase_cNummov  
       LEFT JOIN dbo.CNT_TIPO_MONEDA CTM ON cav.Emp_cCodigo = CTM.Emp_cCodigo AND cav.Ase_cTipoMoneda = CTM.Mon_cCodigo  
